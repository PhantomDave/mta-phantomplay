function initializeCharacterDatabase()
    local result = query("CREATE TABLE IF NOT EXISTS characters (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), age INT, gender VARCHAR(16), account_id INT, FOREIGN KEY (account_id) REFERENCES accounts(id))")
    if result then
...
function CreateCharacter(name, age, gender, accountId)
    if not name or not age or not gender or not accountId then
        outputDebugString("[DEBUG] CreateCharacter called with nil values.")
        return false
    end
    local queryString = string.format("INSERT INTO characters (name, age, gender, account_id) VALUES ('%s', %d, '%s', %d)", name, age, gender, accountId)
    local result = query(queryString)
    return result
end